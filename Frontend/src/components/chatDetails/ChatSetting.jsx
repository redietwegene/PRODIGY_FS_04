...
import { setChatDetailsBox, setLoading } from "../../redux/slices/conditionSlice";
...
const ChatSetting = () => {
    ...
    const handleClearChatCall = () => {
        ...
        fetch(
            `${import.meta.env.VITE_BACKEND_URL}/api/message/clearChat/${selectedChat?._id}`,
            {
                method: "GET",
                headers: {
    ...
    const handleDeleteChatCall = () => {
        ...
        const token = localStorage.getItem("token");
        fetch(
            `${import.meta.env.VITE_BACKEND_URL}/api/chat/deleteGroup/${selectedChat?._id}`,
            {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
    ...
    return (
        <div className="flex flex-col p-4 gap-4 text-white bg-gray-800 border-t border-gray-700 relative h-full overflow-auto">
            ...
            <div
                ...
                className="w-full h-12 border border-gray-600 text-sm rounded-lg flex justify-between items-center p-3 font-medium gap-3 transition-colors cursor-pointer hover:bg-gray-700"
            >
                <h1>Clear Chat</h1>
                <CiCircleInfo
                    fontSize={18}
            ...
            {isConfirm && (
                <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 w-full max-w-md">
                    <div
                        className={`w-full h-16 border border-gray-600 ${
                            isConfirm === "clear-chat"
                                ? "bg-blue-900"
                                : "bg-red-900"
                        } rounded-lg flex justify-between items-center p-4 font-medium gap-4 text-white`}
                    >
                        <h1>
                            {isConfirm === "clear-chat"
                        ...
                        <div className="flex gap-3">
                            <div
                                onClick={() => setConfirm("")}
                                className="border border-gray-600 p-2 rounded-md cursor-pointer hover:bg-gray-700 flex items-center justify-center"
                            >
                                <VscError fontSize={22} />
                            </div>
                            <div
                                onClick={
                                    isConfirm === "clear-chat"
